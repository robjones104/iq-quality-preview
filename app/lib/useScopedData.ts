...
const staticEventBranch = new Map(allEvents.map((e) => [e.id, e.branch]));
...
export function scopeOrders(
  orders: Order[],
  branch: string | null,
  eventBranch: Map<string, string> = staticEventBranch
): Order[] {
 ...
 *  Runtime-created events (intake) extend the order→branch resolution so their
 *  auto-created orders scope correctly too. */
export function useScopedOrders(orders: Order[] = allOrders): Order[] {
  ...
  const branch = caps.branchScoped ? caps.assignedBranch ?? null : null;
  return useMemo(() => {
    const eventBranch = new Map(staticEventBranch);
    Object.values(createdEvents).forEach((e) => eventBranch.set(e.id, e.branch));
    return scopeOrders([...orders, ...Object.values(createdOrders)], branch, eventBranch);
  }, [orders, createdOrders, createdEvents, branch]);
}
