...
**4. Client-simulated side effects.** Everything a server would derive is currently written by the client so screens behave correctly: activity logs, edit history, order logs (`pushActivityLog`, `pushEditHistory`, `pushOrderLog`), tech notifications (replacement #, ship-to changes, invalidation cascades), and the AI summary (700ms fake latency, deterministic content). Photos and attachments are simulated entirely; real upload is a net-new capability (object storage + attachments endpoint), not a wiring exercise.
