...
- **Every Modal keeps an accessible name in every sub-state.** `title={null}` strips the dialog's name (rc-dialog wires `aria-labelledby` only from the title), so success screens render the title inside a visually hidden `.sr-only` span instead of dropping it: screen readers keep the dialog name, sighted users see only the body's own confirmation heading.
